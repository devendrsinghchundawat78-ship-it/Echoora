...
data class GlassEffectConfig(
    ...
    fun isEnabledFor(component: GlassComponent): Boolean =
        globalEnabled && when (component) {
            ...
            GlassComponent.ENHANCED -> enhancedEnabled
        }

    /**
     * Whether the enhanced glass surfaces (search bar, dialogs, sheets, etc.) should
     * render with the liquid glass effect.
     */
    val isEnhancedActive: Boolean
        get() = globalEnabled && enhancedEnabled
}
...
/** The backdrop content (app UI) that glass surfaces sample from. Nullable so that
 *  composables outside the main Scaffold (e.g. Dialogs) can safely read it. */
...
/**
 * Conditionally applies liquid glass when the enhanced glass setting is active.
 * Returns the modifier unchanged if enhanced glass is off, the device does not
 * support it, or the backdrop is not available (e.g. inside a Dialog window).
 * Uses a lighter blur for UI surfaces (search bar, cards, etc.) to maintain readability.
 *
 * Safe to call from any composition context — never crashes.
 */
@Composable
fun Modifier.liquidGlassIfEnhanced(
    shape: CornerBasedShape = RoundedCornerShape(16.dp),
    lightBlur: Float = 4f,
): Modifier {
    val config = LocalGlassEffectConfig.current
    if (!config.isEnhancedActive) return this
    if (!isGlassSupported()) return this
    // Backdrop is nullable — null means we are outside the main Scaffold (e.g. Dialog)
    val backdrop = LocalAppBackdrop.current ?: return this
    // Build a lightweight config with reduced blur for UI surfaces
    val lightConfig = config.copy(
        blurRadius = lightBlur,
        surfaceOpacity = (config.surfaceOpacity * 0.6f).coerceIn(0f, 0.5f),
    )
    return this.liquidGlass(
        config = lightConfig,
        shape = shape,
        applyEdgeEffects = false,
        blurRadiusDp = lightBlur,
    )
}

...
import iad1tya.echo.music.constants.LiquidGlassEnhancedKey
...
fun GlassEffectSettings(
    ...
    val (enhancedGlass, onEnhancedGlassChange) = rememberPreference(
        LiquidGlassEnhancedKey, defaultValue = false
    )
    ...
    Column(
        ...
        Material3SettingsGroup(
            ...
            items = listOf(
                Material3SettingsItem(
                    icon = painterResource(R.drawable.check),
                    title = { Text("Enhanced Liquid Glass") },
                    description = {
                        Text("Apply liquid glass to search bar, dialogs, bottom sheets, and other UI elements")
                    },
                    trailingContent = {
                        Switch(
                            checked = enhancedGlass && globalEnabled && useFloatingNavBar,
                            onCheckedChange = onEnhancedGlassChange,
                            enabled = globalEnabled && useFloatingNavBar,
                            thumbContent = {
                                Icon(
                                    painter = painterResource(
                                        id = if (enhancedGlass && globalEnabled && useFloatingNavBar) R.drawable.check else R.drawable.close
                                    ),
                                    contentDescription = null,
                                    modifier = Modifier.size(SwitchDefaults.IconSize)
                                )
                    ...
                    onClick = { if (globalEnabled && useFloatingNavBar) onEnhancedGlassChange(!enhancedGlass) }
                )
            )
        )
